...
--  Присвоить названия столбцам: InvoiceDate – Дата Продажи; BillingAddress – Адрес Продажи; BillingCity - Город Продажи.
SELECT InvoiceDate AS 'Дата Продажи', BillingAddress AS 'Адрес Продажи',BillingCity AS 'Город Продажи', Total
FROM chinook.invoice
WHERE Total !=17.91
ORDER BY Total DESC;

-- 5.  Покажите фамилии и имена сотрудников компании, нанятых в 2003 году из города Калгари.
SELECT LastName, FirstName, HireDate, City FROM chinook.employee
...
-- 8.  Покажите фамилии и имена сотрудников, занимающих должность менеджера по продажам и ИТ менеджера. 
-- Решите задание двумя способами: используя оператор IN; используя логические операции.
